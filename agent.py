...
class ResumeAnalysisAgent:
    ...
    def extract_text_from_file(self, file):
        """Extract text from a file (PDF or TXT) or directly from text input"""
        # If it's already text (from text area input), return it directly
        ...
        for skill in self.analysis_result.get("missing_skills", []):
            ...
            try:
                ...
                weaknesses.append({
                    "skill": skill,
                    "score": self.analysis_result.get("skill_scores", {}).get(skill, 0),
                    "detail": weakness_content[:200]  # Truncate if it's not JSON
                })  
    ...
    def analyze_resume(self, resume_file, role_requirements=None, custom_jd=None):
        ...
        if custom_jd:
            # Handle both file and text input for custom JD
            if isinstance(custom_jd, str):
                # Direct text input
                self.jd_text = custom_jd
            else:
                # File input
    ...
    def improve_resume(self, improvement_areas, target_role=""):
        ...
        try:
            ...
            if remaining_areas:
                ...
                Skills to focus on: {', '.join(self.extracted_skills) if self.extracted_skills else 'Not specified'}  
    ...
    def get_improved_resume(self, target_role="", highlight_skills=""):
        ...
        try:
            ...
            # FIXED: Use ChatGroq instead of ChatOpenAI
            llm = ChatGroq(model="llama-3.3-70b-versatile", temperature=0.7, api_key=self.api_key)
